...
const containsAnd = (arr) => {
  // Solution code here...
  let stringsAnd = arr.filter(ands => ands.includes('and'));
  return stringsAnd;
};
...
function oddValues(arr) {
  // Solution code here...
  let oddsOnly = arr.filter((oddy) => oddy % 2 === 1);
  return oddsOnly;
}
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let differ = arr.filter(diff => !forbiddenValues.includes(diff)
  );
  return differ;
};
